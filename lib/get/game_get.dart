...
class GameGet extends GetxController {
  final FirebaseGame firebaseGame = Get.put(FirebaseGame());
  final FirebaseLog firebaseLog = Get.put(FirebaseLog());
  final UserDatabase userDatabase = Get.put(UserDatabase());
  final Rx<List<GamePost>> postList = Rx([]);
  final List<GamePost> notFilteredList = [];
  int gameCount = 0;
  final int gameCountIncrement = 10;
  int lastPulledPostCount = 0;
  final RxBool loading = RxBool(false);
  final Rx<List<GameComment>?> currComments = Rx(null);
  ...
  void filterGameList(String filter, GameType? gameType, String? province) {
    postList.value = notFilteredList
        .where((element) =>
            (element.gameType == gameType ||
  ...
  Future<bool> createGame(
      String title, String desc, String province, GameType gameType) async {
    ...
    if (databaseResponse.result) {
      firebaseLog
          .writeLog(databaseResponse.data.id + "  " + LogUtils.createdGame);
    }
  ...
  Future<GameComment?> addComment(String postId, String comment) async {
    try {
      ...
      currComments.refresh();
      await firebaseLog.writeLog(userDatabase.getUser()!.uid! +
          " - " +
          postId +
          " - " +
          LogUtils.addedComment);
      return gameComment;
    } catch (e) {
      return null;
    }
